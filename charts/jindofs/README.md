...
<a name="vdrsi"></a>
### 2.2 配置参数
完整的配置模板如下：<br />这里举例的镜像资源是cn-shanghai区的地址，版本号为3.1.1。**请使用安装配置页面展示的默认的image地址和tag版本。**
...
spec:
    ...
    containers:
    - name: jindofs-demo-app2
      image: "registry.cn-shanghai.aliyuncs.com/jindofs/jindo-fuse:3.1.1"
      command: ["tail"]
      args: ["-f", "/dev/null"]
      env:
...
```
wget https://smartdata-binary.oss-cn-shanghai.aliyuncs.com/b2sdk-3.1.1.tar.gz
tar -zxvf b2sdk-3.1.1.tar.gz
```
