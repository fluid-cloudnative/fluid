...
build()
{
  docker cp ${dev_container_name}:/tmp/alluxio-release-2.7.3-SNAPSHOT-bin.tar.gz /tmp/
  cp /tmp/alluxio-release-2.7.3-SNAPSHOT-bin.tar.gz /alluxio/integration/docker
  ...
  if test -f /alluxio/integration/docker/Dockerfile-dev; then
    docker build -f Dockerfile-dev -t alluxio/alluxio-dev:release-2.7.3-SNAPSHOT-$GIT_COMMIT --build-arg ALLUXIO_TARBALL=alluxio-release-2.7.3-SNAPSHOT-bin.tar.gz --build-arg ENABLE_DYNAMIC_USER="true" .
  else
    docker build -t alluxio/alluxio-dev:release-2.7.3-SNAPSHOT-$GIT_COMMIT --build-arg ALLUXIO_TARBALL=alluxio-release-2.7.3-SNAPSHOT-bin.tar.gz --build-arg ENABLE_DYNAMIC_USER="true" .
  fi
  # Tag Alluxio image
  docker tag alluxio/alluxio-dev:release-2.7.3-SNAPSHOT-$GIT_COMMIT ${alluxio_image_name}:release-2.7.3-SNAPSHOT-$GIT_COMMIT

  # Build Fuse image if needed. Tag Fuse image
  if test -f /alluxio/integration/docker/Dockerfile.fuse; then
    docker build -f Dockerfile.fuse -t alluxio/alluxio-fuse:release-2.7.3-SNAPSHOT-$GIT_COMMIT --build-arg ALLUXIO_TARBALL=alluxio-release-2.7.3-SNAPSHOT-bin.tar.gz --build-arg ENABLE_DYNAMIC_USER="true" .
    docker tag alluxio/alluxio-fuse:release-2.7.3-SNAPSHOT-$GIT_COMMIT ${alluxio_fuse_image_name}:release-2.7.3-SNAPSHOT-$GIT_COMMIT
  else
    docker tag alluxio/alluxio-dev:release-2.7.3-SNAPSHOT-$GIT_COMMIT ${alluxio_fuse_image_name}:release-2.7.3-SNAPSHOT-$GIT_COMMIT
  fi

  docker push ${alluxio_fuse_image_name}:release-2.7.3-SNAPSHOT-$GIT_COMMIT &
  docker push ${alluxio_image_name}:release-2.7.3-SNAPSHOT-$GIT_COMMIT &
}
