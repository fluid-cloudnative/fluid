...
In addition to the above integrated cache engines, users may need to access their own storage systems in cloud scenarios. In order to meet this requirement, users must complete the docking between the self built storage system and the Kubernetes environment by themselves. They need to write a customized CSI plug-in or a Runtime Controller based on the self built storage system to interface with Fluid, which requires users to have relevant knowledge of Kubernetes, thus increasing the work cost of the storage provider.
...
| Storage System| Use Guide | 
|-----------------|:--------------------:|
| NFS           |       [Use Guide](./nfs/en-nfs.md)        |
