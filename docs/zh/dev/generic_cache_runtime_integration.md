...
需要注意的是，Fluid 仍会为该 Dataset 创建 PVC/PV 且状态为 Bound，但业务 Pod 挂载该 PVC 会一直停留在 ContainerCreating（报 timeout waiting for FUSE mount point）。这类缓存系统的业务 Pod 不要挂这个 PVC，应该由应用直接连缓存服务。

应用要连缓存服务，就得知道 Master 的地址这些信息。业务 Pod 加上下面的标签和注解，Fluid 的 webhook 会在创建 Pod 时把运行时配置注入进去：
...
metadata:
  ...
  annotations:
    fluid.io/datasets: "demofs"    # 多个数据集用逗号分隔
```

注解里的每个数据集，Pod 会多出：

- 一个卷 `fluid-runtime-config-<数据集名>`，来自这个数据集的运行时配置 ConfigMap；
- 这个卷挂在 `/etc/fluid/config/<数据集名>/`，只读；
- 一个环境变量 `FLUID_RUNTIME_CONFIG_PATH_<数据集名>`，指向这个目录下的 `runtime.sh`。

环境变量名里的数据集名会转成大写，`-` 换成 `_`，因为 shell 变量名里不能有 `-`。比如数据集 `mooncake-demo` 对应 `FLUID_RUNTIME_CONFIG_PATH_MOONCAKE_DEMO`。

业务容器在自己的 entrypoint 里 `source` 这个文件就行：

```sh
#!/bin/sh
. "$FLUID_RUNTIME_CONFIG_PATH_DEMOFS"

# 地址自己拼；端口不在运行时配置里，由接入方决定
...
`runtime.sh` 里有哪些变量，见后面的「Shell 形式的 RuntimeConfig」。

几点需要注意：

- 注解里的数据集要和业务 Pod 在同一个 namespace。ConfigMap 卷不能跨 namespace 引用，所以注解里只写数据集名。
- 注入只在 Pod 创建时做一次，之后 `volumes` 这些字段就改不了了。注解里的数据集不存在或者还没 Bound 时，Pod 会创建失败，报错里会写明是哪个数据集；Deployment、Job 这类控制器会自己重试，数据集 Bound 之后就能创建成功。
- 如果数据集的运行时配置里没有 `runtime.sh`（比如 cacheruntime-controller 的版本比较旧），Pod 会正常创建，但不会被注入，原因只记在 webhook 的日志里。
- Runtime 之后有变化（比如 Master 扩容），ConfigMap 会更新，kubelet 也会同步容器里的文件，但应用只在启动时 `source` 一次，不会重新读。
- 用 `source` 的方式一次只能用一个数据集。`runtime.sh` 里的变量名不带数据集前缀，连着 `source` 两个，后一个会把前一个覆盖掉。要同时用多个数据集，可以读同目录下的 `runtime.json`，或者每 `source` 一个就先把要用的变量存到别的名字下。

完整配置示例参见 [使用 CacheRuntime 部署 Mooncake](../samples/cacheruntime/mooncake_cache_runtime.md)。
...
#### Shell 形式的 RuntimeConfig

同一个 ConfigMap 里除了 `runtime.json`，还有一份 `runtime.sh`，内容相同，只是写成了 shell 的 `export` 语句，镜像里没有 JSON 解析工具也能直接 `source`。上面那份 `runtime.json` 对应的 `runtime.sh` 是：
...
shell 里没有嵌套结构，字段都被展平了：

- 普通字段是 `前缀_字段名`，比如 `MASTER_NAME`、`MASTER_SERVICE_NAME`。
- 列表先给个数，再逐个编号：`MOUNTS_COUNT` 加 `MOUNT_0_NAME`，`RUNTIME_ACCESSMODES_COUNT` 加 `RUNTIME_ACCESSMODE_0`，分层存储是 `WORKER_TIEREDSTORE_LEVELS_COUNT` 加 `WORKER_TIEREDSTORE_LEVEL_0_QUOTA_0` 这种。
- map（各个 `OPTIONS`）写成一个 JSON 字符串，要用的话自己解析，或者直接读 `runtime.json`。

拓扑里没有声明的组件也会输出同样的一组变量，只是 `ENABLED` 为 `false`，其余为空。判断组件在不在看 `ENABLED` 就行，脚本开了 `set -u` 也不会因为变量不存在而出错。没有 client 时，client 这几行是：
...
值都用单引号括起来。`mountPoint`、`options` 这些是用户填的，放在双引号里 `$` 和反引号会被 shell 展开；值里本身的单引号写成 `'\''`。

组件 Pod 里 `$FLUID_RUNTIME_CONFIG_PATH` 指向 `runtime.json`，`runtime.sh` 在同一个目录下。
