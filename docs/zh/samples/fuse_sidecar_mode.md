...
> **清理脚本**：Fluid 提供了安全的清理脚本 [`tools/clean-sidecar-hostpath.sh`](https://github.com/fluid-cloudnative/fluid/blob/master/tools/clean-sidecar-hostpath.sh)，具有以下安全特性：
> - 仅在目录数超过 1000 时执行清理
> - 仅清理超过 3 个月的目录
> - 检查挂载点，避免删除正在使用的目录
> - 使用 `rmdir` 命令确保目录为空才删除，防止误删数据
>
> 使用示例：
> ```bash
> # 预览将要删除的目录
> sudo ./tools/clean-sidecar-hostpath.sh --dry-run
>
> # 执行清理
> sudo ./tools/clean-sidecar-hostpath.sh
>
> # 自定义配置
> sudo ./tools/clean-sidecar-hostpath.sh --threshold 500 --age-days 60
> ```
