...
check_control_plane_status() {
    ...
    while true; do
        total_pods=$(kubectl get pod -n fluid-system --no-headers 2>/dev/null | grep -cv "Completed" || echo 0)
        running_pods=$(kubectl get pod -n fluid-system --no-headers 2>/dev/null | grep -c "Running" || echo 0)
