...
wait_dataset_bound() {
    ...
    while true; do
        ...
        if [[ "${log_interval}" -eq 3 ]]; then
            ((log_times++))
            syslog "checking dataset.status.phase==Bound (already $((log_times * 3 * 5))s, last state: ${last_state})"
...
wait_job_completed() {
    ...
    local counter=0
    while true; do
        succeed=$(kubectl get job "${job_name}" -ojsonpath='{@.status.succeeded}' || echo "0")
        failed=$(kubectl get job "${job_name}" -ojsonpath='{@.status.failed}' || echo "0")
        
        # Check if succeed or failed is empty (can happen if field is missing)
        [[ -z "${succeed}" ]] && succeed=0
        [[ -z "${failed}" ]] && failed=0
