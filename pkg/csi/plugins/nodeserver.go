...
func (ns *nodeServer) getCleanFuseFunc(volumeId string) (func() error, error) {
	...
	var shouldCleanFuse, shouldCleanMetadata bool
	...
	if updateStrategy == v1alpha1.OnIdleFuseUpdateStrategy {
		if checkIfFuseNeedUpdate(runtimeInfo, latestFuseImageVersion) {
			shouldCleanFuse, shouldCleanMetadata = true, true
		}
	}
	...
	return func() error {
		...
		if shouldCleanMetadata {
			cleanMetadataFuseImageVersionAfterFuseClean(runtimeInfo.GetNamespace(), runtimeInfo.GetName(), runtimeInfo.GetRuntimeType())
		}
		return nil
	}, nil
}
...
func checkIfFuseNeedUpdate(runtimeInfo base.RuntimeInfoInterface, latestFuseImageVersion string) (needUpdate bool) {
	...
	currentImageVersion, found, err := utils.GetFuseImageVersionFromMetadata(runtimeInfo.GetNamespace(), runtimeInfo.GetName(), runtimeInfo.GetRuntimeType())
	...
	if !found || len(currentImageVersion) == 0 {
		return
	}
