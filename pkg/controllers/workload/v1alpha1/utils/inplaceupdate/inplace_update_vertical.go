...
func (v *NativeVerticalUpdate) IsPodQoSChanged(oldTemp, newTemp *v1.PodTemplateSpec) bool {
	...
	if getPodQOS(oldPod) != getPodQOS(newPod) {
		return true
	}
	return false
}

// getPodQOS returns the QoS class of a pod, simplified version
...
// computePodQOS computes the QoS class of a pod based on its resource requests/limits
func computePodQOS(pod *v1.Pod) v1.PodQOSClass {
	...
	for _, container := range allContainers {
		...
		for name, quantity := range container.Resources.Requests {
			if name == v1.ResourceCPU || name == v1.ResourceMemory {
				limitQuantity, exists := container.Resources.Limits[name]
				if !exists || limitQuantity.Cmp(quantity) != 0 {
					isGuaranteed = false
				}
			}
		}
