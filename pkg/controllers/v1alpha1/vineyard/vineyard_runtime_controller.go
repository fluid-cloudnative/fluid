...
const controllerName string = "VineyardRuntimeController"
...
// Reconcile reconciles vineyard runtime
// +kubebuilder:rbac:groups=data.fluid.io,resources=vineyardruntimes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.fluid.io,resources=vineyardruntimes/status,verbs=get;update;patch
...
func (r *RuntimeReconciler) Reconcile(context context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	ctx := cruntime.ReconcileRequestContext{
		Context:        context,
		Log:            r.Log.WithValues("vineyardruntime", req.NamespacedName),
...
func (r *RuntimeReconciler) SetupWithManager(mgr ctrl.Manager, options controller.Options) error {
	return ctrl.NewControllerManagedBy(mgr).
		WithOptions(options).
		For(&datav1alpha1.VineyardRuntime{}).
		Complete(r)
}
