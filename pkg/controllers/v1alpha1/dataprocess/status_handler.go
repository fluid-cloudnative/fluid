...
func (handler *OnceStatusHandler) GetOperationStatus(ctx cruntime.ReconcileRequestContext, object client.Object, opStatus *datav1alpha1.OperationStatus) (result *datav1alpha1.OperationStatus, err error) {
	...
	if err != nil {
		if utils.IgnoreNotFound(err) == nil {
			ctx.Log.Info("Related job missing, will delete helm release and retry", "namespace", ctx.Namespace, "jobName", jobName)
			if err = helm.DeleteReleaseIfExists(releaseName, ctx.Namespace); err != nil {
				ctx.Log.Error(err, "can't delete dataprocess release", "namespace", ctx.Namespace, "releaseName", releaseName)
				return
			}
		}
