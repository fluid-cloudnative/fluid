...
func (r *CacheRuntimeReconciler) Reconcile(context context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	ctx.Runtime = runtime
	// CacheRuntime cannot use ddc.InferEngineImpl because ReferenceDatasetEngine inherits valueFile property
...
func (r *CacheRuntimeReconciler) SetupWithManager(mgr ctrl.Manager, options controller.Options, eventDriven bool) error {
	if eventDriven {
		return watch.SetupWatcherForReconcilerWithDataset(mgr, options, r, common.CacheRuntime)
	} else {
	...
	return cache.Options{
		ByObject: map[client.Object]cache.ByObject{
			&appsv1.StatefulSet{}: {Label: labels.SelectorFromSet(labels.Set{
				common.App: common.CacheRuntime,
			})},
