...
func CreatePersistentVolumeForRuntime(client client.Client,
	...
	if !found {
		...
		pv.Spec.PersistentVolumeSource.CSI.VolumeAttributes[common.AnnotationCheckFuseMountReadyPattern] = common.FusePatternAll

		// set from runtime
		for key, value := range runtime.GetAnnotations() {
			if key == common.AnnotationCheckFuseMountReadyPattern {
				pv.Spec.PersistentVolumeSource.CSI.VolumeAttributes[common.AnnotationCheckFuseMountReadyPattern] = value
			}
		}
