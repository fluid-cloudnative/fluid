...
func (ssc *defaultStatefulSetControl) updateStatefulSet(
	...
	*status.CollisionCount = collisionCount
	...
	//这个切片用于存放所有符合条件的 Pods。条件是 0 <= getOrdinal(pod) < set.Spec.Replicas
	//且 Pod 不在 deleteSlots 中。也就是说，这个切片包含的 Pods 是符合副本数量要求的，并且没有被标记为需要删除的 Pods。
	replicas := make([]*v1.Pod, replicaCount)
	// slice that will contain all Pods such that set.Spec.Replicas <= getOrdinal(pod) or exist in deleteSlots
	//这个切片用于存放所有被标记为需要删除的 Pods。具体条件是 set.Spec.Replicas <= getOrdinal(pod)
	//或者 Pods 存在于 deleteSlots 中。也就是说，这个切片包含的 Pods 要么超出了期望的副本数量，要么已经被标记为需要删除。
	...
	// updates.
	// 此时，所有当前的副本都在运行并且就绪，我们可以考虑终止它们。
	// 我们会等待所有前驱副本都在运行并且就绪后再尝试删除。
	// 我们将以单调递减的顺序终止 Pods，范围在 [len(pods), set.Spec.Replicas) 之间。
	// 注意，在这个时间段内我们不会复活 Pods。另请注意，缩放操作将优先于更新操作。
