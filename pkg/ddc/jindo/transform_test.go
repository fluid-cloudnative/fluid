...
func TestTransformHostPID(t *testing.T) {
	resources := corev1.ResourceRequirements{}
	resources.Limits = make(corev1.ResourceList)
	resources.Limits[corev1.ResourceMemory] = resource.MustParse("2Gi")
	...
	for _, test := range tests {
		engine := &JindoEngine{Log: log.NullLogger{}}
		engine.transformHostPID(test.runtime, test.jindoValue)
		if test.jindoValue.UseHostPID != test.expect {
			t.Errorf("expected value %v, but got %v", test.expect, test.jindoValue.UseHostPID)
		}
	}
}
...
func TestTransformAllocatePorts(t *testing.T) {
	...
	for _, test := range tests {
		engine := &JindoEngine{Log: log.NullLogger{}}
		engine.transformNetworkMode(test.runtime, test.jindoValue)
		engine.allocatePorts(test.jindoValue)
		if test.jindoValue.Master.Port.Rpc != test.expect {
			t.Errorf("expected value %v, but got %v", test.expect, test.jindoValue.Master.Port.Rpc)
		}
	}
}
