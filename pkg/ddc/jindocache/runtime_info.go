...
func (e *JindoCacheEngine) getRuntimeInfo() (base.RuntimeInfoInterface, error) {
	if e.runtimeInfo == nil {
		...
		if err != nil {
			if len(runtime.GetOwnerReferences()) > 0 {
				e.runtimeInfo.SetNamespacedNameAlias(runtime.GetOwnerReferences()[0].UID)
			}
		...
		e.runtimeInfo.SetNamespacedNameAlias(dataset.GetUID())
