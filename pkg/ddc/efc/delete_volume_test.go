...
func newTestEFCEngine(client client.Client, name string, namespace string, withRuntimeInfo bool) *EFCEngine {
	...
	runTimeInfo, _ := base.BuildRuntimeInfo(name, namespace, common.EFCRuntimeType, datav1alpha1.TieredStore{})
	if !withRuntimeInfo {
		runTimeInfo = nil
		runTime = nil
	}
...
func TestEFCEngine_DeleteVolume(t *testing.T) {
	...
	eacEngineCommon := newTestEFCEngine(fakeClient, "eacdemo", "fluid", true)
	eacEngineErr := newTestEFCEngine(fakeClient, "error", "fluid", true)
	eacEngineNoRunTime := newTestEFCEngine(fakeClient, "eacdemo", "fluid", false)
