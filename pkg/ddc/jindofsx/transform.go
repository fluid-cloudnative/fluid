...
func (e *JindoFSxEngine) transformMaster(runtime *datav1alpha1.JindoRuntime, metaPath string, value *Jindo, dataset *datav1alpha1.Dataset, secretMountSupport bool) (err error) {
	...
	for _, tmpMount := range dataset.Spec.Mounts {
		...
		if strings.HasPrefix(mount.MountPoint, "oss://") {
			...
			propertiesFileStore[buildBucketPropertyKey(jindoFSxOSSBucketPrefix, ossBucketName, jindoEndpointSuffix)] = mount.Options["fs.oss.endpoint"]
			if strings.Contains(mount.Options["fs.oss.endpoint"], "dls") {
				propertiesFileStore[buildBucketPropertyKey(jindoFSxOSSBucketPrefix, ossBucketName, jindoDLSStorageEnableSuffix)] = "true"
			}
			...
			if !hasOSSAccessKeyIDRef && mount.Options["fs.oss.accessKeyId"] != "" {
				propertiesFileStore[buildBucketPropertyKey(jindoFSxOSSBucketPrefix, ossBucketName, jindoAccessKeyIDSuffix)] = mount.Options["fs.oss.accessKeyId"]
			}
			if !hasOSSAccessKeySecretRef && mount.Options["fs.oss.accessKeySecret"] != "" {
				propertiesFileStore[buildBucketPropertyKey(jindoFSxOSSBucketPrefix, ossBucketName, jindoAccessKeySecretSuffix)] = mount.Options["fs.oss.accessKeySecret"]
			}
		}
		...
		for _, encryptOption := range mount.EncryptOptions {
			...
			if mountType == "oss" && ossBucketName != "" {
				...
				if key == "fs.oss.accessKeyId" {
					propertiesFileStore[buildBucketPropertyKey(jindoFSxOSSBucketPrefix, ossBucketName, jindoAccessKeyIDSuffix)] = secretValue
				}
				if key == "fs.oss.accessKeySecret" {
					propertiesFileStore[buildBucketPropertyKey(jindoFSxOSSBucketPrefix, ossBucketName, jindoAccessKeySecretSuffix)] = secretValue
				}
...
func (e *JindoFSxEngine) transformFuse(runtime *datav1alpha1.JindoRuntime, value *Jindo) {
	...
	if endpoint, ok := extractSingleOSSEndpoint(value.Master.FileStoreProperties, jindoFSxOSSBucketPrefix); ok {
...
func (e *JindoFSxEngine) transformLogConfig(runtime *datav1alpha1.JindoRuntime, value *Jindo) {
	...
	if len(value.BucketSecretPaths) != 0 {
		...
		fusePropreties["fs.oss.provider.format"] = jindoSecretProviderFormat
		for bucketName, secretURI := range value.BucketSecretPaths {
			setBucketSecretProviderProperties(fusePropreties, "fs", bucketName, secretURI)
			fusePropreties["aliyun.oss.bucket."+bucketName+".provider.url"] = secretURI
			if endpoint, ok := value.Master.FileStoreProperties[buildBucketPropertyKey(jindoFSxOSSBucketPrefix, bucketName, jindoEndpointSuffix)]; ok {
...
func (e *JindoFSxEngine) transformToken(value *Jindo) {
	...
	if len(value.BucketSecretPaths) != 0 {
		properties["default.credential.provider"] = "secrets:///token/"
		properties["jindofsx."+value.MountType+".provider.endpoint"] = "secrets:///token/"
		properties["jindofsx."+value.MountType+".provider.format"] = jindoSecretProviderFormat
	}
	for bucketName, secretURI := range value.BucketSecretPaths {
		setBucketSecretProviderProperties(properties, "jindofsx", bucketName, secretURI)
	}
	if endpoint, ok := extractSingleOSSEndpoint(value.Master.FileStoreProperties, jindoFSxOSSBucketPrefix); ok {
		properties["jindofsx.oss.endpoint"] = endpoint
	}
