...
func (e *JindoFSxEngine) queryCacheStatus() (states cacheStates, err error) {
	...
	if len(e.runtime.Spec.TieredStore.Levels) > 0 && e.runtime.Spec.TieredStore.Levels[0].MediumType == "MEM" {
		totalCapacityLabel = SummaryPrefixTotalMemCapacity
		usedCapacityLabel = SummaryPrefixUsedMemCapacity
	} else {
		totalCapacityLabel = SummaryPrefixTotalCapacity
		usedCapacityLabel = SummaryPrefixUsedCapacity
	}
