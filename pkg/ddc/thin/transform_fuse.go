...
func (t *ThinEngine) parseHostVolumeFromDataset(dataset *datav1alpha1.Dataset, thinValue *ThinValue) error {
	...
	for _, mount := range dataset.Spec.Mounts {
		for key, value := range mount.Options {
			...
			if len(subStrings) > 2 {
				return fmt.Errorf("invalid dataset option format, %s: %s", key, value)
			}
			...
			thinValue.Fuse.Volumes = append(thinValue.Fuse.Volumes, corev1.Volume{
				Name: fmt.Sprintf("fluid-fuse-hostvolume-%v", index),
