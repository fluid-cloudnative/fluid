...
func TestAlluxioEngine_setFinalPortToValue(t *testing.T) {
	type fields struct {
		runtime                *datav1alpha1.AlluxioRuntime
		name                   string
		namespace              string
	...
	var finalPort int = 20000
	var oldPort int = 20001
	tests := []struct {
		name   string
		fields fields
		args   args
	}{
		{
			...
			args: args{
				runtime: &datav1alpha1.AlluxioRuntime{
					Spec: datav1alpha1.AlluxioRuntimeSpec{
						Properties: map[string]string{
							"alluxio.master.rpc.port": strconv.Itoa(finalPort),
							"alluxio.master.web.port": strconv.Itoa(finalPort),
							"alluxio.worker.rpc.port": strconv.Itoa(finalPort),
							"alluxio.worker.web.port": strconv.Itoa(finalPort),
							"alluxio.job.master.rpc.port": strconv.Itoa(finalPort),
							"alluxio.job.master.web.port": strconv.Itoa(finalPort),
							"alluxio.job.worker.rpc.port": strconv.Itoa(finalPort),
							"alluxio.job.worker.web.port": strconv.Itoa(finalPort),
							"alluxio.job.worker.data.port": strconv.Itoa(finalPort),
							"alluxio.proxy.web.port": strconv.Itoa(finalPort),
							"alluxio.master.embedded.journal.port": strconv.Itoa(finalPort),
							"alluxio.job.master.embedded.journal.port": strconv.Itoa(finalPort),
						},
					},
				},
				alluxioValue: &Alluxio{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			e.setPortProperties(tt.args.runtime, tt.args.alluxioValue)
			key := tt.args.alluxioValue.Master.Ports.Rpc
			if key != finalPort {
				t.Errorf("expected %d, got %d", finalPort, key)
			}
		})
	}
}
