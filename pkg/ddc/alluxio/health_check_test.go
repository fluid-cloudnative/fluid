...
func TestCheckExistenceOfEngine(t *testing.T) {
	var statefulsetInputs = []appsv1.StatefulSet{
		{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "hbase-master",
	...
	for _, test := range testCase {
		klog.Info("test")
		err := test.engine.CheckExistenceOfEngine()
		if err != nil && test.expectedErrorNil == true ||
			err == nil && test.expectedErrorNil == false {
			t.Errorf("fail to exec the CheckExistenceOfEngine function with err %v", err)
			return
		}
