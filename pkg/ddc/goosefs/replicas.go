...
func (e *GooseFSEngine) SyncReplicas(ctx cruntime.ReconcileRequestContext) (err error) {
	...
	if desireReplicas > runtime.Status.CurrentWorkerNumberScheduled {
		...
		ctx.Recorder.Eventf(runtime, corev1.EventTypeNormal, common.Succeed, "GooseFS runtime scaled out. current replicas: %d, desired replicas: %d.", currentReplicas, desireReplicas)
		...
		e.Log.Info("Scaling in GooseFS workers", "expectedReplicas", replicas)
		...
		if curReplicas > replicas {
			ctx.Recorder.Eventf(runtime, corev1.EventTypeWarning, common.RuntimeScaleInFailed,
				"GooseFS workers are being used by some pods, can't scale in (expected replicas: %v, current replicas: %v)",
				replicas, curReplicas)
		} else {
			ctx.Recorder.Eventf(runtime, corev1.EventTypeNormal, common.Succeed, "GooseFS runtime scaled in. current replicas: %d, desired replicas: %d.", curReplicas, desireReplicas)
		}
