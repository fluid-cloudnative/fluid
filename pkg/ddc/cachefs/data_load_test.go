...
configs:
  ...
  formatCmd: /usr/local/bin/cachefs format --trash-days=0 --access-key=${ACCESS_KEY}
...
fuse:
  metricsPort: 14001
  command: /bin/mount.cachefs ${METAURL} /runtime-mnt/cachefs/default/jfsdemo/cachefs-fuse
    -o metrics=0.0.0.0:9567,cache-size=1024,free-space-ratio=0.1,cache-dir=/jfs/cache3:/jfs/cache4
  criticalPod: true
  enabled: true
  hostMountPath: /runtime-mnt/cachefs/default/jfsdemo
  hostNetwork: true
  image: registry.cn-hangzhou.aliyuncs.com/cachefs/cachefs-fuse
  imagePullPolicy: IfNotPresent
  imageTag: v1.0.0-4.8.0
  mountPath: /runtime-mnt/cachefs/default/jfsdemo/cachefs-fuse
  nodeSelector:
    fluid.io/f-default-jfsdemo: "true"
  privileged: true
  resources: {}
  statCmd: stat -c %i /runtime-mnt/cachefs/default/jfsdemo/cachefs-fuse
  volumeMounts:
...
func TestCachefsFSEngine_CheckExistenceOfPath(t *testing.T) {
	configMap := &v1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-dataset-cachefs-values",
			Namespace: "fluid",
	...
	if !(err == nil && notExist == false) {
		t.Errorf("fail to exec the function %s: %t", err, notExist)
	}
	wrappedUnhook()
}
