...
func NewCacheFSFileUtils(podName string, containerName string, namespace string, log logr.Logger) CacheFSFileUtils {
	return CacheFSFileUtils{
		podName:   podName,
		namespace: namespace,
...
func (j *CacheFSFileUtils) LoadMetadataWithoutTimeout(juicefsPath string) (err error) {
	var (
...
// IsExist checks if the cachefsPath exists
func (j *CacheFSFileUtils) IsExist(cachefsPath string) (found bool, err error) {
	var (
		command = []string{"ls", cachefsPath}
		stdout  string
		stderr  string
	)
...
func (j *CacheFSFileUtils) Count(juiceSubPath string) (total int64, err error) {
	var (
...
func (j *CacheFSFileUtils) GetFileCount(juiceSubPath string) (fileCount int64, err error) {
	var (
...
func (j *CacheFSFileUtils) Mkdir(juiceSubPath string) (err error) {
	var (
...
func (j *CacheFSFileUtils) DeleteCacheDirs(dirs []string) (err error) {
...
func (j *CacheFSFileUtils) DeleteCacheDir(dir string) (err error) {
	// cache dir check
	match := ValidCacheDir(dir)
	if !match {
		j.log.Info("invalid cache directory, skip cleaning up", "cacheDir", dir)
...
func (j *CacheFSFileUtils) GetStatus(source string) (status string, err error) {
	var (
...
func (j *CacheFSFileUtils) GetMetric(cachefsPath string) (metrics string, err error) {
	var (
...
func (j *CacheFSFileUtils) GetUsedSpace(juicefsPath string) (usedSpace int64, err error) {
	var (
...
// exec with timeout
func (j *CacheFSFileUtils) exec(command []string, verbose bool) (stdout string, stderr string, err error) {
...
// execWithoutTimeout
func (j *CacheFSFileUtils) execWithoutTimeout(command []string, verbose bool) (stdout string, stderr string, err error) {
...
func (j *CacheFSFileUtils) QueryMetaDataInfoIntoFile(key KeyOfMetaDataFile, filename string) (value string, err error) {
	line := ""
	switch key {
	case DatasetName:
		line = "1p"
