...
var _ = Describe("factory", func() {
	...
	Describe("InferEngineImpl", func() {
		...
		It("still works when the dataset name itself contains values", func() {
			impl := InferEngineImpl(fluidv1alpha1.RuntimeStatus{ValueFileConfigmap: "my-values-dataset-goosefs-values"}, common.ThinEngineImpl)
		...
		It("falls back to default when only a non-values suffix contains a registered engine", func() {
			impl := InferEngineImpl(fluidv1alpha1.RuntimeStatus{ValueFileConfigmap: "dataset-values-alluxio-config"}, common.ThinEngineImpl)
	...
	Describe("buildFuncMap", func() {
		...
		It("uses canonical dataset-engine-values names for every registered engine", func() {
			expectedEngines := []string{
				common.AlluxioEngineImpl,
				common.JindoFSEngineImpl,
				common.JindoFSxEngineImpl,
			...
			for _, engine := range expectedEngines {
				configmapName := fmt.Sprintf("dataset-%s-values", engine)
				Expect(InferEngineImpl(fluidv1alpha1.RuntimeStatus{ValueFileConfigmap: configmapName}, common.ThinEngineImpl)).To(Equal(engine))
			}
		})
	})
})
