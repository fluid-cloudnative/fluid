...
func TestVineyardEngine_DeleteVolume(t *testing.T) {
	...
	vineyardEngineCommon := newTestVineyardEngine(fakeClient, "hbase", "fluid", true)
	vineyardEngineErr := newTestVineyardEngine(fakeClient, "error", "fluid", true)
	vineyardEngineNoRunTime := newTestVineyardEngine(fakeClient, "hbase", "fluid", false)
	var testCases = []TestCase{
		{
			engine:    vineyardEngineCommon,
			isDeleted: true,
			isErr:     false,
		},
		{
			engine:    vineyardEngineErr,
			isDeleted: true,
			isErr:     true,
		},
		{
			engine:    vineyardEngineNoRunTime,
			isDeleted: true,
			isErr:     true,
		},
	}
...
func TestVineyardEngine_DeleteFusePersistentVolume(t *testing.T) {
	testPVInputs := []*v1.PersistentVolume{
		{
			ObjectMeta: metav1.ObjectMeta{
				Name: "fluid-hbase",
	...
	fakeClient := fake.NewFakeClientWithScheme(testScheme, tests...)
	vineyardEngine := newTestVineyardEngine(fakeClient, "hbase", "fluid", true)
	vineyardEngineNoRuntime := newTestVineyardEngine(fakeClient, "hbase", "fluid", false)
	testCases := []TestCase{
		{
			engine:    vineyardEngine,
...
func TestVineyardEngine_DeleteFusePersistentVolumeClaim(t *testing.T) {
	testPVCInputs := []*v1.PersistentVolumeClaim{
		{
			ObjectMeta: metav1.ObjectMeta{
				Name:       "hbase",
