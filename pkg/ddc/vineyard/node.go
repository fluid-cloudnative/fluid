...
func (e *VineyardEngine) SyncScheduleInfoToCacheNodes() (err error) {
	...
	workerSelector, err := labels.Parse(fmt.Sprintf("fluid.io/dataset=%s-%s,app=vineyard,role=vineyard-worker", e.namespace, e.name))
	if err != nil {
		return err
	}
...
func (e *VineyardEngine) getAssignedNodes() (nodeNames []string, err error) {
	var (
